  ...
  return (
    <section className="min-w-full max-w-full mx-auto ">
      <section className="flex justify-center items-center bg-[url('/PageHeroBG.svg')] bg-cover bg-no-repeat bg-center w-full min-w-full h-80">
        <h2 className={`font-bold text-2xl md:text-3xl ${cairo.className} my-10`}>
          ABOUT POWERLINES
        </h2>
      </section>
      ...
      <section className="flex flex-wrap bg-white p-5 md:p-20  mb-[90px] md:mb-[150px] mt-10">
        ...
        <section className="w-full order-1 md:order-1  md:hidden">
          <h2 className={`text-xl md:text-3xl font-bold mb-4 text-darker-yellow ${cairo.className}`}>OUR MISSION</h2>
        </section>
        {/* 
Text Column  */}
        <section className="w-full md:w-1/2 flex items-center justify-center h-auto order-3 md:order-2 ">
          <section className=" mt-0 md:mt-8">
            <h2 className={`text-xl md:text-3xl font-bold mb-4 text-darker-yellow hidden md:block ${cairo.className}`}>OUR MISSION</h2>
            <p className={`text-xl md:text-4xl font-semibold leading-8 mb-8  ${cairo.className}`}>PowerLines aims to reshape our energy system by accelerating clean energy and equity through utility regulation.</p> <p className={`text-base md:text-xl font-normal leading-9 ${robotoSlab.className}`}>We seek to create a world in which clean, affordable, and reliable energy systems contribute to safe, healthy, and vibrant communities.</p>
