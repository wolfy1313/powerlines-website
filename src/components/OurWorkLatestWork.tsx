...
const Section: React.FC<SectionProps> = ({ title, content, download, link }) => {
  ...
  return (
    <section className={`${josefinSans.className}  grid grid-cols-1 md:place-content-between mx-6  border-t-2 border-b-2 font-medium overflow-hidden transition-[max-height] duration-500 ease-in-out ${isDropdownVisible ? "md:max-h-96" : "max-h-[4.7rem] md:max-h-20"}`} >
...
function OurWorkLatestWork() {
  return (
    <div className='flex flex-col border-b-4'>
      <div className='pt-16 pb-10 border-b-4 mx-6'>
